-- Exact hybrid retrieval baseline: English full-text plus exact cosine candidates.
...
WITH params AS (
  SELECT
    p_collection_slug AS collection_slug,
    NULLIF(trim(p_query), '') AS query_text,
    p_embedding AS query_embedding,
    GREATEST(1, LEAST(p_limit, 50)) AS result_limit
),
base AS (
  SELECT
    c.id AS chunk_id,
  ...
  WHERE co.slug = p.collection_slug
    AND sv.status = 'active'
    AND (
      p_snapshot_id IS NULL
      OR EXISTS (
        SELECT 1
        FROM atlas.corpus_snapshots AS snapshot
...
vector_candidates AS (
  SELECT
    e.chunk_id,
    row_number() OVER (ORDER BY e.embedding <=> e.query_embedding, e.chunk_id)::integer AS vector_rank
  FROM embedded AS e
...
fused AS (
  SELECT
    candidate.chunk_id,
    min(candidate.keyword_rank) AS keyword_rank,
    min(candidate.vector_rank) AS vector_rank,
    sum(
      1.0 / (60.0 + COALESCE(candidate.keyword_rank, candidate.vector_rank))
    ) AS fused_score
  FROM (
...
SELECT
  ...
  o.fused_rank
FROM ordered AS o
JOIN base AS b ON b.chunk_id = o.chunk_id
CROSS JOIN params AS p
WHERE o.fused_rank <= p.result_limit
ORDER BY o.fused_rank;
$$;
