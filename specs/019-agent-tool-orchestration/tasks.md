...
- [ ] T043 [US3] Add an explicit idempotency key to plan/run/approval requests and enforce ownership, scope, quota, consent, and replay protection before any private or mutating adapter can execute (FR-006, FR-007; partial). Evidence: client/server idempotency keys, actor ownership, consent, and session-backed scope rejection are covered; durable approval-key binding and quota enforcement remain.
