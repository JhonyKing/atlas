...
- [X] T021 [US2] Implement durable run/plan/call/event/checkpoint persistence, sequence checks, and replay-safe resume in `apps/backend/src/atlas/persistence/agent_runs.py` and `apps/backend/src/atlas/agent/checkpoints.py`. Evidence: PostgreSQL persistence contract suite **12 passed**, including cross-repository run/event replay and checkpoint claims.
...
- [X] T038 Run full lint/typecheck/unit/integration/security/browser and deterministic evaluation gates; record evidence for each user story. Evidence: `docs/verification/019-agent-tool-orchestration.md`; backend 428 passed/4 conditional skips, Ruff, strict mypy over 183 files, deterministic agent gate 58 passed plus 5/5 dataset cases, web typecheck/ESLint/Vitest 35/35, Chromium 149 passed/4 hosted-deployment skips, and a successful Next.js 16.2.12 production build.
...
- [X] T043 [US3] Add an explicit idempotency key to plan/run/approval requests and enforce ownership, scope, quota, consent, and replay protection before any private or mutating adapter can execute (FR-006, FR-007). Evidence: one operation key is bound into the durable approval decision key and reused by the web client across plan/approval/run; session scope, consent, ownership and replay gates remain fail-closed; the rolling per-visitor/tool quota uses the PostgreSQL idempotency ledger in non-development runtimes and rejects exhaustion before adapter execution. Focused result: 66 agent/contract tests, 7 policy/quota tests, Ruff, strict mypy (183 files), web typecheck/ESLint, and 35/35 web tests passed.
...
- [X] T045 [US2] Complete read-only result adapters so answer, comparison, report, news, and corpus outputs preserve provenance, source versions, bounded excerpts, typed evidence relations, and artifact links (FR-005, FR-009). Evidence: normalized envelopes are persisted in `agent_runs.output`; queued answer/comparison/report artifacts expose safe status/download links; daily-news and corpus snapshot contract coverage passes in `apps/backend/tests/contract/agent/test_tool_orchestration.py`; adapter boundary tests preserve source versions, excerpts, and typed relations.
