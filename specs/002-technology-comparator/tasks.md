...
- [ ] T038 [US1] Wire a production-safe comparison run service into `create_runtime_app()` with the separate quota, verified snapshot selection, retrieval/workflow, persistence, trace tree and terminal SSE events per FR-CMP-001–FR-CMP-010. (Progress: runtime now wires quota, snapshot, persistence and fail-closed terminal SSE service; real retrieval/workflow executor and trace-tree integration remain.)
