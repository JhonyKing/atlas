...
- [ ] T038 [US1] Wire a production-safe comparison run service into `create_runtime_app()` with the separate quota, verified snapshot selection, retrieval/workflow, persistence, trace tree and terminal SSE events per FR-CMP-001–FR-CMP-010. (Progress: runtime now wires quota, snapshot, persistence, fail-closed terminal SSE service and LangSmith comparison/retrieval/verification traces; real retrieval/workflow executor remains.)
