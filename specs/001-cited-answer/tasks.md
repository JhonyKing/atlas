...
- [ ] T013 Implement versioned Alembic/PostgreSQL migrations for schemas, least-privilege roles, `vector`, `pgmq`, `pg_cron`, and common timestamp/UUID helpers in `database/migrations/` to satisfy T012
- [ ] T014 [P] Write failing domain-schema tests for Question, AnswerDraft, Claim, Evidence, Citation, CorpusStatus, and controlled errors in `apps/backend/tests/unit/domain/test_schemas.py`
- [ ] T015 Implement strict Pydantic domain contracts and enums in `apps/backend/src/atlas/domain/` to satisfy T014 without importing provider SDK types
- [ ] T016 [P] Write failing provider-port and deterministic-fake tests in `apps/backend/tests/unit/providers/test_ports.py`
- [ ] T017 Implement `AnswerGenerator`, `EmbeddingProvider`, clock, pricing, and source-fetch ports plus deterministic fakes in `apps/backend/src/atlas/providers/ports.py` and `apps/backend/tests/fakes/`
...
- [ ] T047 [US1] Implement `POST /v1/answers`, `GET /v1/answers/{run_id}`, and repeat-safe cancellation through `DELETE /v1/answers/{run_id}` in `apps/backend/src/atlas/api/routes/answers.py` with invalid-input handling, request deduplication, quota, controlled errors, cancellation, and no provisional claim text
