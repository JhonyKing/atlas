...
- [ ] T046 [US2] Select Cloud Run as the managed runtime and add secret-reference-only, immutable-image API service and worker-pool templates, a dry-run-by-default guarded deployment command, contract tests, ADR/runbook updates, and redacted preflight evidence. Keep T032 open until real billable resources are owner-authorized and verified.
