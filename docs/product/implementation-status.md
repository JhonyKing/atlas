...
| 019 | Agent tool orchestration | Planner estructurado GPT-5.6 Luna con fallback determinista, catálogo, plan, aprobación, ejecución bounded, eventos, UI, persistencia Postgres no-development con replay entre repositorios, idempotencia durable, envelopes de evidencia read-only, enlaces de artefactos, trazas de ciclo de vida y evidencia live sin contenido privado | 2 tareas SpecKit pendientes: políticas y gates de despliegue | `specs/019-agent-tool-orchestration/tasks.md` |
...
`anon`/`authenticated`. T049 queda cerrada y Feature 019 pasa de 7 a 2 tareas abiertas tras los
slices de replay durable, envelopes read-only y trazas live. Supabase
