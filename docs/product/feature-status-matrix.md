...
| 006 — Agent graph, checkpoints and human review | Typed classification/planning, explicit routes, safe checkpoints, idempotent replay, review API/panel and migration 0024 are implemented in the current slice. | Agent verification command **29 passed**, Ruff/mypy, migration/SQL contract, full backend regression **253 passed, 4 skipped**, and review journey **1 passed**; evidence is in `docs/verification/006-agent-graph-human-review.md`. | Node timeout enforcement, provider-backed graph integration, crash/resume integration and final convergence remain. | **In progress** — remaining tasks are tracked in `specs/006-agent-graph-human-review/tasks.md`. |
