...
def test_repository_contains_one_ordered_24_revision_chain() -> None:
    ...
    assert manifest[-1].revision_id == "0024_agent_checkpoints_reviews"
...
def test_manifest_rejects_an_unexpected_revision_count(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Expected 25 migrations"):
        load_migration_manifest(tmp_path, expected_count=25)
