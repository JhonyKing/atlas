...
def test_repository_contains_one_ordered_31_revision_chain() -> None:
    ...
    assert manifest[-1].revision_id == "agent_tool_rls"
...
def test_manifest_rejects_an_unexpected_revision_count(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Expected 32 migrations"):
        load_migration_manifest(tmp_path, expected_count=32)
