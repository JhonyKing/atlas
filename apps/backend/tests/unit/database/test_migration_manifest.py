...
def test_repository_contains_one_ordered_30_revision_chain() -> None:
    ...
    assert manifest[-1].revision_id == "agent_checkpoint_claims"
...
def test_manifest_rejects_an_unexpected_revision_count(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Expected 31 migrations"):
        load_migration_manifest(tmp_path, expected_count=31)
